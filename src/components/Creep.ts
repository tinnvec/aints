...
Creep.prototype.run = function(this: Creep) {
  ...
  if (this.isSearching && this.stepsFromLastSite >= Config.MAX_SEARCH_STEPS) {
    this.lastDirection = undefined
    this.isSearching = false
  }
...
Creep.prototype.depositPheromone = function(this: Creep): number {
  ...
  const maxDepositAmount = Config.MAX_TILE_PHEROMONE_LEVEL -
    (this.stepsFromLastSite * Config.MIN_PHEROMONE_DEPOSIT_LEVEL)
